...
function calculator(n, numA, numB) {
  let line = "";
  const first = numA.toString(2).padStart(n, "0"); // 2진법 변환 후, n 자리수 만큼 앞에서 0으로 채움
  const second = numB.toString(2).padStart(n, "0"); // 2진법 변환 후, n 자리수 만큼 앞에서 0으로 채움

  for (let i = 0; i < n; i++) {
    if (first[i] === "1" || second[i] === "1")
      line += "#"; // 둘 중 하나라도 1이면 #, 아니면 ' '
    else line += " ";
  }
  return line;
}
...
// NOTE
기존에 풀었던 코드에서 더 나은 방법을 고민하다가 padStart(), padEnd() 메서드에 대해서 알게됨!
